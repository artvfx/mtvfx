...
const Header: React.FC<HeaderProps> = ({ page, setPage }) => {
  ...
  const baseNavLinks = [
    ...
    { name: "Solutions", href: "#", action: () => setPage('solutions') },
    { name: "About Us", href: "#", action: () => setPage('about') },
    { name: "Contact", href: "#", action: () => setPage('contact') },
  ];

  // Conditionally add a "Home" link if not on the home page
  const navLinks = page !== 'home' 
    ? [{ name: "Home", href: "#", action: () => setPage('home') }, ...baseNavLinks] 
    : baseNavLinks;


  const handleLinkClick = (e: React.MouseEvent<HTMLAnchorElement>, link: typeof navLinks[0]) => {
    // For single-page app navigation links
    if (['Home', 'CREATE', 'About Us', 'Contact', 'Solutions'].includes(link.name)) {
      e.preventDefault();
      link.action();
    }
    // For links to sections on the home page (if any were left)
    else {
      if (page !== 'home') {
